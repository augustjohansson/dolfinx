...
void assemble_cells(
    ...
    const graph::AdjacencyList<T>& coeffs, const std::vector<T>& constants,
    ...
    const std::vector<std::uint32_t>& cell_info)
{
  ...
  for (std::int32_t c : active_cells)
  {
    ...
    kernel(Ae.data(), coeffs.links(c).data(), constants.data(),
    ...
    const Eigen::Array<std::uint8_t, Eigen::Dynamic, Eigen::Dynamic>& perms)
{
  ...
  for (std::int32_t f : active_facets)
  {
    ...
    // Tabulate tensor
    std::fill(Ae.begin(), Ae.end(), 0);
    kernel(Ae.data(), coeffs.links(cells[0]).data(), constants.data(),
